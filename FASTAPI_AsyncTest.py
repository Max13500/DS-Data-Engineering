...
if __name__ == '__main__':
    print('making 50 requests on the `/sync` endpoint ...')
    delta_t = overflow_requests(compute_response_time_sync, 50)
    print('took {} seconds'.format(delta_t))

    print('making 50 requests on the `/async` endpoint')
    delta_t = overflow_requests(compute_response_time_async, 50)
    print('took {} seconds'.format(delta_t))